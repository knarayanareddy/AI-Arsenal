#!/usr/bin/env node
// Migrates the inference-engines/ phase of the projects-vertical
// reorganisation: moves the 7 canonical inference-engine project entries
// into content/projects/inference-engines/. No merges/consolidations
...
const enrichment = JSON.parse(await fs.readFile('.migration/proj-inference-engines.json', 'utf8'));

// No merges needed for this phase.
const MERGES = {};

// Original file locations for the 7 canonical inference-engine entries.
const SOURCE_FILES = {
  'llama-cpp': 'content/projects/llms/inference-engines/llama-cpp.md',
  llamafile: 'content/projects/llms/inference-engines/llamafile.md',
  lmdeploy: 'content/projects/llms/inference-engines/lmdeploy.md',
  ollama: 'content/projects/llms/inference-engines/ollama.md',
  sglang: 'content/projects/llms/inference-engines/sglang.md',
  'text-generation-inference': 'content/projects/llms/inference-engines/text-generation-inference.md',
  vllm: 'content/projects/llms/inference-engines/vllm.md'
};

const DEST_DIR = 'content/projects/inference-engines';

function buildFrontmatter(data, e) {
  const next = { ...data };
  next.phase = 'inference-engine';
...
async function run() {
  ...
  console.log(chalk.green(`${dryRun ? '[DRY RUN] ' : ''}Migrated ${Object.keys(enrichment).length} inference-engine entries; deleted ${Object.keys(MERGES).length} merged duplicates.`));
  return moves;
}
