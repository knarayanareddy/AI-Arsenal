#!/usr/bin/env node
// Migrates the agent-systems/ phase of the projects-vertical
// reorganisation: moves the 2 canonical agent-system project entries into
// content/projects/agent-systems/. No merges/consolidations needed.
//
// Usage: node scripts/migrate-projects-agent-systems.js [--dry-run]
...
const enrichment = JSON.parse(await fs.readFile('.migration/proj-agent-systems.json', 'utf8'));

// No merges needed for this phase.
const MERGES = {};

// Original file locations for the 2 canonical agent-system entries.
const SOURCE_FILES = {
  insforge: 'content/projects/agents/coding-agents/insforge.md',
  stagehand: 'content/projects/agents/browser-agents/stagehand.md'
};

const DEST_DIR = 'content/projects/agent-systems';
...
async function run() {
  ...
  console.log(chalk.green(`${dryRun ? '[DRY RUN] ' : ''}Migrated ${Object.keys(enrichment).length} agent-system entries; deleted ${Object.keys(MERGES).length} merged duplicates.`));
  return moves;
}
