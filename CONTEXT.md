...
Generated: 2026-09-14T15:50:25.546Z | Entries: 1063 | Schema version: 1.0.0
...
### agents
- Hermes Agent (⭐245393, score:55) — Open-source personal agent platform with tools, memory, skills, subagents, and multi-channel runtimes
- AutoGPT (⭐187323, score:55) — Autonomous agent platform and classic agent project for accessible AI automation
- Dify (⭐155693, score:55) — Visual platform for building agentic workflows, RAG apps, chatbots, and AI automations
- Browser Use (⭐114595, score:55) — The most-starred open-source browser agent: connects LLMs to a real browser so agents can navigate, fill forms and complete web tasks autonomously
- OpenHands (⭐87874, score:55) — AI software engineering agent platform for coding, terminal work, browser actions, and automation

### code-generation
- screenshot-to-code (⭐78794, score:55) — Converts screenshots, mockups, and Figma designs into working frontend code (HTML/Tailwind, React, Vue) using multimodal LLMs — with video-to-prototype support
- Continue (⭐35903, score:55) — Open-source AI coding assistant for VS Code and JetBrains — chat, autocomplete, edit, and agent modes over any model, including fully local
- Open Code Review (⭐24954, score:55) — Alibaba's Go/Node-distributed AI code-review CLI combining deterministic file pipelines with an LLM agent for line-level findings
- Mistral Vibe (⭐4949, score:34) — Mistral's open-source CLI coding assistant with subagents, delegated tasks, skills, and a voice mode
- Tabby (⭐33881, score:31) — Self-hosted AI coding assistant server in Rust — an on-prem Copilot alternative bundling its own model serving, code RAG, and IDE integrations

### computer-vision
- PaddleOCR (⭐89507, score:55) — Baidu's industrial OCR and document-AI toolkit: 80+ language text recognition, layout parsing, and lightweight models that run from server to edge
- Tesseract OCR (⭐76480, score:55) — The long-standing open-source OCR engine that recognizes text in 100+ languages using an LSTM line recognizer, widely used as the default OCR backend
- Ultralytics YOLO (⭐61590, score:55) — The most widely used real-time object detection framework: YOLO models for detection, segmentation, pose, and tracking with a three-line API
- Supervision (⭐49997, score:55) — Roboflow's model-agnostic CV utilities — one Detections API over any detector, plus annotators, zone/line analytics, tracking, and dataset tools
- OCRmyPDF (⭐34760, score:55) — A command-line tool that adds a searchable OCR text layer to scanned PDFs using Tesseract while preserving the original page images and metadata

### data-pipelines
- DuckDB (⭐41219, score:55) — An in-process analytical SQL database that runs fast columnar OLAP queries directly on files (Parquet, CSV, Arrow) without a server
- Marker (⭐39739, score:55) — Deep-learning PDF-to-markdown converter that handles tables, equations, and layout with optional LLM-assisted accuracy boosts
- Polars (⭐39723, score:55) — A fast, multi-threaded DataFrame library in Rust with a lazy query optimizer and Arrow memory model, a high-performance alternative to pandas for AI/ML data
- LangExtract (⭐38578, score:55) — Python library for grounded structured extraction from unstructured text with source spans and visualization
- Liteparse (⭐12304, score:55) — A fast open-source document parser from LlamaIndex, written in Rust, that converts PDFs and documents into structured, LLM-ready output

### evaluation
- Terminal-Bench (⭐2581, score:27) — Benchmark measuring AI agents on real end-to-end tasks in a sandboxed terminal environment, from compiling code to training models
- lmms-eval (⭐4404, score:23) — Multimodal evaluation toolkit spanning text, image, video, and audio tasks and model adapters
- MTEB (⭐3421, score:21) — The Massive Text Embedding Benchmark — the standard evaluation suite and leaderboard for embedding and reranker models across 1000+ tasks
- LightEval (⭐2541, score:20) — Hugging Face's all-in-one LLM evaluation toolkit for running benchmarks across multiple inference backends with reproducible
...
### llms
- Ollama (⭐180915, score:55) — Local runtime for downloading, running, and serving open-weight models on developer machines
- llama.cpp (⭐128197, score:55) — C and C++ inference engine for running GGUF-quantized LLMs locally and on edge devices
- DeepSeek-V3 / R1 (⭐104445, score:55) — DeepSeek open-weight MoE and reasoning model family known for strong cost-performance
- vLLM (⭐91730, score:55) — High-throughput inference and serving engine for LLMs with batching and OpenAI-compatible APIs
- LobeChat (LobeHub) (⭐82468, score:55) — Self-hostable, multi-provider AI chat platform with plugins, agents marketplace, knowledge base, and one-click deployment

### multimodal
- Stable Diffusion WebUI (⭐164935, score:55) — AUTOMATIC1111's browser-based application for local Stable Diffusion image generation with an extensive extension ecosystem for control, upscaling
- ComfyUI (⭐133087, score:55) — Node-graph engine for visual generative AI: the standard open-source interface for building diffusion and video-generation pipelines
- Open-Sora (⭐29784, score:55) — Open-source text/image-to-video generation model and training pipeline from HPC-AI Tech, aiming for an accessible reproduction of Sora-style video synthesis
- MiniCPM-V (⭐26374, score:55) — Efficient open vision-language model series from OpenBMB that runs strong image/video/OCR understanding on-device, including phones
- Hunyuan3D-2 (⭐14851, score:55) — Tencent's open model for generating high-resolution textured 3D assets from images or text using a two-stage shape-then-texture diffusion pipeline

### observability
- Langfuse (⭐34593, score:55) — Open-source LLM observability platform for traces, evals, prompts, metrics, and datasets
- Opik (⭐22011, score:55) — Open-source Comet platform for LLM tracing, evaluation, prompt optimization, and dashboards
- DeepEval (⭐18265, score:55) — An open-source evaluation framework for testing LLM applications in CI
- Portkey AI Gateway (⭐12988, score:55) — A fast open-source AI gateway that routes requests to 1,600+ LLMs behind one API, with built-in guardrails, retries, fallbacks, caching, and observability
- Phoenix (⭐11457, score:55) — Arize Phoenix open-source observability and evaluation platform for LLM, RAG, and agent systems

### rag
- Firecrawl (⭐180298, score:55) — Open-source and hosted web scraping API that turns websites into LLM-ready markdown/data
- LangChain (⭐146304, score:55) — A framework for composing LLM applications, retrieval flows, tools, and agents
- RAGFlow (⭐90676, score:55) — Open-source RAG engine combining document understanding, retrieval, and agent capabilities
- Crawl4AI (⭐83466, score:55) — Open-source crawler and scraper designed to produce LLM-friendly web content
- Docling (⭐66398, score:55) — IBM-origin open-source toolkit for parsing and exporting documents for generative AI

### tooling
- Supabase (⭐109169, score:55) — Open-source backend platform: Postgres database, auth, storage, and realtime APIs
- Context7 (⭐62003, score:55) — Up-to-date code documentation platform for LLMs and AI coding editors through retrieval and MCP access
- Cherry Studio (⭐51788, score:55) — Cross-platform desktop LLM client supporting many cloud and local providers, with assistants, knowledge bases, MCP tools, and artifacts in one app
- UI-TARS Desktop (⭐38974, score:55) — Open-source desktop application and agent stack for computer-use and browser automation driven by vision-language GUI-grounding models
- SillyTavern (⭐33352, score:55) — Self-hosted, extensible chat frontend for local and hosted LLMs, focused on character personas, long conversations, and power-user control over prompts

### voice-audio
- Whisper (⭐109092, score:55) — OpenAI's open-source speech recognition model: robust multilingual transcription and translation trained on 680k hours of audio
- GPT-SoVITS (⭐61779, score:55) — Few-shot voice cloning and TTS toolkit that clones a voice from ~1 minute of audio, with WebUI for data prep, training, and inference
- whisper.cpp (⭐53658, score:55) — A dependency-free C/C++ port of OpenAI's Whisper that runs speech-to-text on CPU and consumer hardware via the ggml/GGUF tensor library
- OpenVoice (⭐37524, score:55) — An instant voice-cloning model from MIT and MyShell that separates tone color from a base speaker, enabling cross-lingual cloning and style control
- VoxCPM (⭐37257, score:55) — A tokenizer-free text-to-speech model that generates speech in a continuous acoustic space for multilingual synthesis, creative voice design
